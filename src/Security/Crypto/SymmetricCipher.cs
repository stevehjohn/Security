...
{
    public byte[] Encrypt(byte[] secret, byte[] key, byte[] iv, byte[] salt = null)
    {
        var cipherParameters = salt == null
            ? new AeadParameters(new KeyParameter(key), 128, iv)
            : new AeadParameters(new KeyParameter(key), 128, iv, salt);
    ...
    public byte[] Decrypt(byte[] cipherText, byte[] key, byte[] iv, byte[] salt = null)
    {
        var cipherParameters = salt == null
            ? new AeadParameters(new KeyParameter(key), 128, iv)
            : new AeadParameters(new KeyParameter(key), 128, iv, salt);
